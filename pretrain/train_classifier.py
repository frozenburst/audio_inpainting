...
class hp:
    ...
    save_descript = '_coarse_net_weightedLoss_noIr_25_35_pretrain'
    debug_graph = False
...
if __name__ == "__main__":
    ...
    with strategy.scope():
        ...
        def train_step(inputs):
            ...
            #model_input = x_incomplete

            with tf.GradientTape() as tape:
                # Generator
                x_stage1 = model(inputs=model_input, training=True)
                x_complete = x_stage1 * mask + x_incomplete
                # l1 loss
                l1_loss_diff = tf.math.abs(x_pos - x_stage1)
                masked_l1_diff = tf.math.abs(x_pos - x_complete)
                if hp.weighted_loss:
                    l1_loss_diff = mag_mel_weighted_map(l1_loss_diff)
                    masked_l1_diff = mag_mel_weighted_map(masked_l1_diff)
                l1_loss = compute_global_loss(tf.math.reduce_mean(l1_loss_diff))
                masked_l1 = compute_global_loss(tf.math.reduce_mean(masked_l1_diff))
            ...
            loss['masked_l1'] = masked_l1
            loss['loss_to_x1'] = gradient_calc(l1_loss, x_stage1)
            loss['masked_to_x1'] = gradient_calc(masked_l1, x_stage1)
        ...
        # Save model
        print("Save model.")
        model.save("pretrain_models/first_stage")
